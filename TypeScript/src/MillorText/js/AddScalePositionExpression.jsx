...
},{}],"AddScalePositionExpression.ts":[function(require,module,exports) {
"use strict";
...
var AddScalePositionExpression = /*#__PURE__*/function () {
  function AddScalePositionExpression() {
    _classCallCheck(this, AddScalePositionExpression);
    this.scriptName = this.constructor.name;
  }
  _createClass(AddScalePositionExpression, [{
    key: "main",
    value: function main() {
      if (!Common_1.Common.isActiveComp()) return;
      var acriveComp = app.project.activeItem;
      var sourceTextLayer = this.extractSourceTextLayer(acriveComp.layers);
      var trfVal = Common_1.Common.getLayerTransformValue(sourceTextLayer);
      this.addScalePositionExpression(acriveComp.selectedLayers, trfVal.rectWithScale);
    }
  }, {
    ...
    key: "addScalePositionExpression",
    value: function addScalePositionExpression(i_layers, i_sourceTextRectWithScale) {
      i_layers.forEach(function (layer) {
        var rect = Common_1.Common.getGrobalCoordinateRect(layer);
        layer.selectedProperties.forEach(function (property) {
          ...
          var posX = value[0];
          var posY = value[1];
          var templateDiffX = posX - (rect.left + rect.width / 2);
          var templateDiffY = posY - (rect.top + rect.height / 2);
          var expression = Common_1.Common.loadExpression(ExpressionFile_1.ScalePosition.FILE_PATH);
          expression = expression.replace(ExpressionFile_1.ScalePosition.TEXT_WIDTH_WITH_SCALE, i_sourceTextRectWithScale.width.toString());
          expression = expression.replace(ExpressionFile_1.ScalePosition.TEXT_HEIGHT_WITH_SCALE, i_sourceTextRectWithScale.height.toString());
          expression = expression.replace(ExpressionFile_1.ScalePosition.POS_X, posX.toString());
          expression = expression.replace(ExpressionFile_1.ScalePosition.POS_Y, posY.toString());
          expression = expression.replace(ExpressionFile_1.ScalePosition.DIFF_X_FROM_CENTER, templateDiffX.toString());
          expression = expression.replace(ExpressionFile_1.ScalePosition.DIFF_Y_FROM_CENTER, templateDiffY.toString());
          property.expression = expression;
        });
      });
    }
  }]);
  return AddScalePositionExpression;
}();
var addScalePositionExpression = new AddScalePositionExpression();
app.beginUndoGroup(addScalePositionExpression.scriptName);
addScalePositionExpression.main();
app.endUndoGroup();
},{"../../Common/Common":"../../Common/Common.ts","./ExpressionFile":"ExpressionFile.ts"}]},{},["AddScalePositionExpression.ts"], null)