...
},{"../../node_modules/extendscript-es5-shim-ts":"../../../node_modules/extendscript-es5-shim-ts/index.js"}],"../../Common/PropertyName.ts":[function(require,module,exports) {
"use strict";
...
var PropertyName = /*#__PURE__*/_createClass(function PropertyName() {
  _classCallCheck(this, PropertyName);
});
PropertyName.AVLayer = {
  Transform: "ADBE Transform Group"
};
PropertyName.ShapeLayer = {
  Contents: "Contents",
...
},{}],"../../Common/CommandId.ts":[function(require,module,exports) {
"use strict";
...
},{}],"CopyTextTemplate.ts":[function(require,module,exports) {
"use strict";
...
var PropertyName_1 = require("../../Common/PropertyName");
var CommandId_1 = require("../../Common/CommandId");
var ExpressionFile_1 = require("./ExpressionFile");
var CopyTextTemplate = /*#__PURE__*/function () {
  function CopyTextTemplate() {
    _classCallCheck(this, CopyTextTemplate);
    this.IS_DEBUG = false;
    this.scriptName = this.constructor.name;
  }
  _createClass(CopyTextTemplate, [{
    key: "main",
    value: function main() {
      if (!Common_1.Common.isActiveComp()) return;
      var acriveComp = app.project.activeItem;
      var textLayers = this.extractSelectedTextLayers(acriveComp);
      if (textLayers.length == 0) {
        alert("Select TextLayer.");
        return;
      }
      var templateComps = this.extractTextTemplateComps();
      if (Object.keys(templateComps).length === 0) {
        alert("Not exist template comp.");
        return;
      }
      var palette = this.showUI(textLayers, templateComps);
      if (palette == null) {
    ...
    key: "extractSelectedTextLayers",
    value: function extractSelectedTextLayers(i_comp) {
      var textLayers = [];
      var selectedLayers = i_comp.selectedLayers;
      selectedLayers.forEach(function (selectedLayer) {
    ...
    key: "extractTextTemplateComps",
    value: function extractTextTemplateComps() {
      var textTemplateComps = {};
      var allItems = app.project.items;
      for (var i = 1; i <= allItems.length; i++) {
        var item = allItems[i];
        if (!(item instanceof CompItem)) continue;
        var compName = item.name;
        if (compName.match(ExpressionFile_1.CommonDef.SOURCE_COMP_PREFIX)) {
    ...
    key: "showUI",
    value: function showUI(i_selectedTextLayers, i_templateComps) {
      var _this = this;
      var palette = new Window("palette", this.scriptName, undefined, {
        resizeable: true
      });
      if (palette == null) return null;
      var group = palette.add("group");
      group.orientation = "column";
      group.alignment = ["fill", "top"];
      group.alignChildren = ["left", "top"];
      group.spacing = 5;
      group.margins = 0;
      var text = group.add("statictext");
      text.alignment = ["left", "center"];
      text.text = "Select template composition.";
      var drpDwn = group.add("dropdownlist");
      drpDwn.alignment = ["fill", "top"];
      for (var key in i_templateComps) {
        drpDwn.add("item", key);
      }
      ...
      if (this.IS_DEBUG) {
        ...
        okBtn.onClick = function () {
          palette.close();
          _this.onOKClick(i_selectedTextLayers, i_templateComps, drpDwn, checkBox);
        };
      }
    ...
    key: "onOKClick",
    value: function onOKClick(i_selectedTextLayers, i_templateComps, i_dropDownList, i_checkBox) {
      if (i_dropDownList.selection == null || typeof i_dropDownList.selection == "number") {
        alert("Error to get selected item.");
        return;
      }
      var templateCompName = i_dropDownList.selection.text;
      var isCopyPropertyLink = i_checkBox.value;
      app.beginUndoGroup(this.scriptName);
      this.replaceTextLayer2TemplateComp(i_selectedTextLayers, i_templateComps, templateCompName, isCopyPropertyLink);
      app.endUndoGroup();
    }
  }, {
    key: "forDebug",
    value: function forDebug(i_selectedTextLayers, i_templateComps) {
      var templateCompName = "TXT_test";
      var isCopyPropertyLink = true;
      app.beginUndoGroup(this.scriptName);
      this.replaceTextLayer2TemplateComp(i_selectedTextLayers, i_templateComps, templateCompName, isCopyPropertyLink);
      app.endUndoGroup();
    }
  }, {
    key: "replaceTextLayer2TemplateComp",
    value: function replaceTextLayer2TemplateComp(i_selectedTextLayers, i_templateComps, i_templateCompName, i_isCopyPropertyLink) {
      var _this2 = this;
      var templateComp = i_templateComps[i_templateCompName];
      i_selectedTextLayers.forEach(function (originalLayer) {
        var oriLayerInfo = _this2.getTextLayerInfo(originalLayer);
        var newComp;
        if (i_isCopyPropertyLink) {
          newComp = _this2.copyCompWithPropertyLinks(templateComp, oriLayerInfo.containingFolder, oriLayerInfo.text);
          _this2.cancelPropertyLinks(templateComp, newComp);
          _this2.addMillorTextExpression(newComp, templateComp.name);
        } else {
          newComp = _this2.copyComp(templateComp, oriLayerInfo.containingFolder, oriLayerInfo.text);
        }
        var sourceTextLayer = _this2.extractSourceTextLayer(newComp.layers);
        var srcLayerInfo = _this2.getTextLayerInfo(sourceTextLayer);
        _this2.updateText(sourceTextLayer, oriLayerInfo.text);
        var fontSizeRatio = _this2.culcFontSizeRatio(sourceTextLayer, originalLayer);
        _this2.copyLayerTransform(newComp, srcLayerInfo.transformValue, oriLayerInfo.transformValue, fontSizeRatio);
        var newLayer = oriLayerInfo.containingComp.layers.add(newComp);
        newLayer.outPoint = oriLayerInfo.duration;
        newLayer.startTime = oriLayerInfo.inPoint;
        newLayer.moveBefore(originalLayer);
        newLayer.label = originalLayer.label;
        originalLayer.remove();
      });
    }
  }, {
    key: "getTextLayerInfo",
    value: function getTextLayerInfo(i_textLayer) {
      var text = i_textLayer.sourceText.value.text;
      var containingComp = i_textLayer.containingComp;
      var containingFolder = containingComp.parentFolder;
      var transformValue = Common_1.Common.getLayerTransformValue(i_textLayer);
      var inPoint = i_textLayer.inPoint;
      var duration = i_textLayer.outPoint - i_textLayer.inPoint;
      var textLayerInfo = {
        text: text,
    ...
    key: "copyComp",
    value: function copyComp(i_sourceComp, i_targetFolder, i_newCompName) {
      var newComp = this.duplicateComp(i_sourceComp, i_targetFolder, i_newCompName);
      this.deleteMarker(newComp);
      return newComp;
    }
  }, {
    key: "copyCompWithPropertyLinks",
    value: function copyCompWithPropertyLinks(i_sourceComp, i_targetFolder, i_newCompName) {
      var newComp = this.duplicateComp(i_sourceComp, i_targetFolder, i_newCompName);
    ...
    key: "duplicateComp",
    value: function duplicateComp(i_sourceComp, i_targetFolder, i_newCompName) {
      var newComp = i_sourceComp.duplicate();
    ...
    key: "deleteAllLayer",
    value: function deleteAllLayer(i_comp) {
      for (var l = i_comp.numLayers; l > 0; l--) {
        i_comp.layer(l).remove();
      }
    }
  }, {
    key: "copyAllLayerWithPropertyLinks",
    value: function copyAllLayerWithPropertyLinks(i_sourceComp, i_targetComp) {
      this.openInViewer(i_sourceComp);
      app.executeCommand(CommandId_1.CommandId.SelectAll);
      app.endUndoGroup();
      app.executeCommand(CommandId_1.CommandId.CopywithPropertyLinks);
      app.beginUndoGroup(this.scriptName);
      app.executeCommand(CommandId_1.CommandId.Close);
      this.openInViewer(i_targetComp);
      app.executeCommand(CommandId_1.CommandId.Paste);
      app.executeCommand(CommandId_1.CommandId.Close);
    }
  }, {
    key: "openInViewer",
    value: function openInViewer(i_comp) {
      i_comp.openInViewer();
      i_comp.openInViewer();
      i_comp.openInViewer();
      i_comp.openInViewer();
    ...
    key: "copyLayerName",
    value: function copyLayerName(i_sourceComp, i_targetComp) {
      for (var l = 1; l <= i_sourceComp.numLayers; l++) {
        i_targetComp.layer(l).name = i_sourceComp.layer(l).name;
      }
    }
  }, {
    key: "deleteMarker",
    value: function deleteMarker(i_comp) {
      for (var l = i_comp.numLayers; l > 0; l--) {
        var layer = i_comp.layer(l);
        var markerKey = layer.marker.numKeys;
        for (var m = 1; m <= markerKey; m++) {
          layer.marker.removeKey(m);
        }
      }
    }
  }, {
    key: "cancelPropertyLinks",
    value: function cancelPropertyLinks(i_sourceComp, i_targetComp) {
      for (var l = 1; l <= i_sourceComp.numLayers; l++) {
        var souceLayer = i_sourceComp.layer(l);
        var targetLayer = i_targetComp.layer(l);
        for (var p = 1; p <= souceLayer.numProperties; p++) {
          this.adjustmentPropertyExpression(souceLayer.property(p), targetLayer.property(p));
        }
      }
    }
  }, {
    key: "adjustmentPropertyExpression",
    value: function adjustmentPropertyExpression(i_sourceProps, i_targetProps) {
      ...
      for (var p = 1; p <= i_sourceProps.numProperties; p++) {
        this.adjustmentPropertyExpression(i_sourceProps.property(p), i_targetProps.property(p));
      }
    }
  }, {
    key: "enableExpression",
    value: function enableExpression(i_prop) {
      if (i_prop.matchName == PropertyName_1.PropertyName.AVLayer.Transform) {
        for (var i = 1; i <= i_prop.numProperties; i++) {
          var childProp = i_prop.property(i);
          if (childProp instanceof Property && childProp.canSetExpression) {
    ...
    key: "copyExpression",
    value: function copyExpression(i_sourceProp, i_targetProp) {
      if (i_sourceProp instanceof Property && i_targetProp instanceof Property) {
    ...
    key: "addMillorTextExpression",
    value: function addMillorTextExpression(i_targetComp, i_sourceCompName) {
      for (var l = 1; l <= i_targetComp.numLayers; l++) {
        var layer = i_targetComp.layer(l);
        if (!(layer instanceof TextLayer)) continue;
        var isSourceText = layer.name == ExpressionFile_1.CommonDef.SOURCE_TEXT_LAYER_NAME;
        var expressionFile = isSourceText ? ExpressionFile_1.MillorLayer.MILLOR_TEMPLATE_FILE_PATH : ExpressionFile_1.MillorLayer.MILLOR_SOURCE_FILE_PATH;
        var expression = Common_1.Common.loadExpression(expressionFile);
        expression = expression.replace(ExpressionFile_1.MillorLayer.COMP_NAME, i_sourceCompName);
        if (!isSourceText) expression = expression.replace(ExpressionFile_1.MillorLayer.LAYER_ID, l.toString());
        layer.sourceText.expression = expression;
      }
    }
  }, {
    key: "extractSourceTextLayer",
    value: function extractSourceTextLayer(i_layers) {
      for (var l = 1; l <= i_layers.length; l++) {
        var layer = i_layers[l];
    ...
    key: "updateText",
    value: function updateText(i_textLayer, i_text) {
      var newText = new TextDocument(i_text);
      i_textLayer.sourceText.setValue(newText);
    }
  }, {
    key: "culcFontSizeRatio",
    value: function culcFontSizeRatio(i_sourceTextLayer, i_originalTextLayer) {
      var sourceFontSize = i_sourceTextLayer.sourceText.value.fontSize;
      var originalFontSize = i_originalTextLayer.sourceText.value.fontSize;
      return originalFontSize / sourceFontSize;
    }
  }, {
    key: "copyLayerTransform",
    value: function copyLayerTransform(i_sourceLayerContainingComp, i_sourceLayerTransform, i_originalLayerTransform, i_fontSizeRatio) {
      var parent3DLayer = i_sourceLayerContainingComp.layers.addText();
      parent3DLayer.threeDLayer = true;
      parent3DLayer.scale.setValue(i_sourceLayerTransform.scaleValue);
      parent3DLayer.anchorPoint.setValue(i_sourceLayerTransform.anchorPointValue);
      ...
      Common_1.Common.setParentLayer(parent3DLayer, i_sourceLayerContainingComp.layers);
      parent3DLayer.scale.setValue([i_originalLayerTransform.scaleValue[0] * i_fontSizeRatio, i_originalLayerTransform.scaleValue[1] * i_fontSizeRatio, i_originalLayerTransform.scaleValue[2] * i_fontSizeRatio]);
      parent3DLayer.anchorPoint.setValue(i_originalLayerTransform.anchorPointValue);
  ...
  return CopyTextTemplate;
}();
var copyTextTemplate = new CopyTextTemplate();
app.beginUndoGroup(copyTextTemplate.scriptName);
copyTextTemplate.main();
app.endUndoGroup();
},{"../../Common/Common":"../../Common/Common.ts","../../Common/PropertyName":"../../Common/PropertyName.ts","../../Common/CommandId":"../../Common/CommandId.ts","./ExpressionFile":"ExpressionFile.ts"}]},{},["CopyTextTemplate.ts"], null)