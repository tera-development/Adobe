...
},{}],"AddMillorTextExpression.ts":[function(require,module,exports) {
"use strict";
...
var AddMillorTextExpression = /*#__PURE__*/function () {
  function AddMillorTextExpression() {
    _classCallCheck(this, AddMillorTextExpression);
    this.TEXT = "ReferenceText\nReferenceText";
    this.PARENT_FOLDER = "03_Common";
    this.scriptName = this.constructor.name;
  }
  _createClass(AddMillorTextExpression, [{
    key: "main",
    value: function main() {
      var _this = this;
      ...
      var selectedTextLayers = this.extractSelectedTextLayers(acriveComp);
      if (selectedTextLayers.length == 0) {
        var textLayer = acriveComp.layers.addText();
        this.addExp(textLayer);
      } else {
        selectedTextLayers.forEach(function (textLayer) {
          _this.addExp(textLayer);
        });
      }
    }
  }, {
    key: "addReferenceTextComp",
    value: function addReferenceTextComp() {
      if (this.existReferenceTextComp()) return;
      var referenceTextComp = app.project.items.addComp(ExpressionFile_1.MillorText.COMP_NAME, 1920, 1080, 1, 10, 30);
      var parentFolder = this.getParentFolder();
      if (parentFolder != null) {
        referenceTextComp.parentFolder = parentFolder;
      }
      var textLayer = referenceTextComp.layers.addText();
      textLayer.name = ExpressionFile_1.MillorText.LAYER_NAME;
      var newText = new TextDocument(this.TEXT);
      textLayer.sourceText.setValue(newText);
    }
  }, {
    key: "existReferenceTextComp",
    value: function existReferenceTextComp() {
      var existReferenceTextComp = false;
      var allItems = app.project.items;
      for (var i = 1; i <= allItems.length; i++) {
        var item = allItems[i];
        if (item instanceof CompItem && item.name == ExpressionFile_1.MillorText.COMP_NAME) {
    ...
    key: "getParentFolder",
    value: function getParentFolder() {
      var parentFolder;
      var allItems = app.project.items;
      for (var i = 1; i <= allItems.length; i++) {
        var item = allItems[i];
    ...
    key: "addExp",
    value: function addExp(i_textLayer) {
      var expression = Common_1.Common.loadExpression(ExpressionFile_1.MillorText.FILE_NAME);
      i_textLayer.sourceText.expression = expression;
    }
  }]);
  return AddMillorTextExpression;
}();
var addMillorTextExpression = new AddMillorTextExpression();
app.beginUndoGroup(addMillorTextExpression.scriptName);
addMillorTextExpression.main();
app.endUndoGroup();
},{"../../Common/Common":"../../Common/Common.ts","./ExpressionFile":"ExpressionFile.ts"}]},{},["AddMillorTextExpression.ts"], null)