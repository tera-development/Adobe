...
},{"../../node_modules/extendscript-es5-shim-ts":"../../../node_modules/extendscript-es5-shim-ts/index.js"}],"AlignCenterPos.ts":[function(require,module,exports) {
"use strict";
...
var KbarArg = /*#__PURE__*/_createClass(function KbarArg() {
  _classCallCheck(this, KbarArg);
});
KbarArg.x = "X";
KbarArg.y = "Y";
var AlignCenterPos = /*#__PURE__*/function () {
  function AlignCenterPos() {
    _classCallCheck(this, AlignCenterPos);
    this.scriptName = this.constructor.name;
  }
  _createClass(AlignCenterPos, [{
    key: "main",
    value: function main() {
      var _this = this;
      if (!Common_1.Common.isActiveComp()) return;
      var acriveComp = app.project.activeItem;
      var selectedLayers = acriveComp.selectedLayers;
      var arg = kbar.button.argument;
      var isX = arg === KbarArg.x;
      var targetPos = 0;
      selectedLayers.forEach(function (layer, i) {
        if (i == 0) {
          targetPos = _this.culcCenterPos(layer, isX);
        } else {
          _this.moveCenterPos(layer, targetPos, isX);
        }
      });
    }
  }, {
    key: "culcCenterPos",
    value: function culcCenterPos(i_layer, i_isX) {
      var centerPos = 0;
      var transform = Common_1.Common.getLayerTransformValue(i_layer);
      if (i_layer instanceof ShapeLayer || i_layer instanceof TextLayer) {
        if (i_isX) {
          centerPos = transform.rectWithScale.left + transform.rectWithScale.width / 2 + transform.positionValue[0] - transform.anchorPointValueWithScale[0];
        } else {
          centerPos = transform.rectWithScale.top + transform.rectWithScale.height / 2 + transform.positionValue[1] - transform.anchorPointValueWithScale[1];
        }
      } else {
        if (i_isX) {
          centerPos = transform.positionValue[0] - transform.anchorPointValueWithScale[0] + transform.rectWithScale.width / 2;
        } else {
          centerPos = transform.positionValue[1] - transform.anchorPointValueWithScale[1] + transform.rectWithScale.height / 2;
        }
      }
      return centerPos;
    }
    ...
    key: "moveCenterPos",
    value: function moveCenterPos(i_layer, i_tergetPos, i_isX) {
      var currentCenterPos = this.culcCenterPos(i_layer, i_isX);
      var posDiff = i_tergetPos - currentCenterPos;
      var transform = Common_1.Common.getLayerTransformValue(i_layer);
      if (i_isX) {
        var newPosX = transform.positionValue[0] + posDiff;
        i_layer.position.setValue([newPosX, transform.positionValue[1], transform.positionValue[2]]);
      } else {
        var newPosY = transform.positionValue[1] + posDiff;
        i_layer.position.setValue([transform.positionValue[0], newPosY, transform.positionValue[2]]);
      }
    }
  }]);
  return AlignCenterPos;
}();
var alignCenterPos = new AlignCenterPos();
app.beginUndoGroup(alignCenterPos.scriptName);
alignCenterPos.main();
app.endUndoGroup();
},{"../../Common/Common":"../../Common/Common.ts"}]},{},["AlignCenterPos.ts"], null)